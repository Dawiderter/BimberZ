...
fn scene(p : vec3f) -> f32 {
    let p_t = transpose(mat3x3(cos(data.r), 0.0, sin(data.r), 0.0, 1.0, 0.0, -sin(data.r), 0.0, cos(data.r))) * p;

    let sphere_0 = torus(p_t, vec2f(1.0, 0.5));

    let sphere_1 = sphere(p_t, 0.2);
